...
# Use settings.HELPDESK_API_URL which should point to the external Help Desk service
BASE_URL = getattr(settings, "HELPDESK_API_URL", os.getenv("HELPDESK_API_URL", "http://165.22.247.50:5001/"))
...
def fetch_resource_by_id(resource_name, resource_id):
    ...
    if not resource_id:
        return None
    # Help Desk API uses /api/ prefix
    url = _build_url(f"api/{resource_name}/{resource_id}/")
    try:
...
def fetch_resource_list(resource_name, params=None, skip_api_prefix=False):
    ...
    params = params or {}
    # Help Desk API uses /api/ prefix
    paths = ['api/' + resource_name + '/']
    if skip_api_prefix:
        paths.append(resource_name + '/')
